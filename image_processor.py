...
class ImageProcessor:
    ...
    def fix_angle(self):
        dy = self.markers[1][1] - self.markers[0][1]
        dx = self.markers[1][0] - self.markers[0][0]
        angle = math.degrees(math.atan2(dy, dx))
        self.debug_print(f"angle: {angle}")    
        center = (self.width//2, self.height//2)
        
        M = cv2.getRotationMatrix2D(center, angle, 1.0)
        self.img = cv2.warpAffine(self.img, M, (self.width, self.height), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_REPLICATE)
    ...
    def extract_cells(self):
        contours, _ = cv2.findContours(self.table_skeleton, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    def prepare_data_for_cnn(self, cell_rescaled):
        micro_pad = 5
        cell_bin = cv2.adaptiveThreshold(src=cell_rescaled,
                                             maxValue=255,
                                             adaptiveMethod=cv2.ADAPTIVE_THRESH_MEAN_C, 
                                             thresholdType=cv2.THRESH_BINARY_INV, 
                                             blockSize=17, # blockSize%2==1
                                             C=7) # Noise filter const
            
        cell_contour, _ = cv2.findContours(image=cell_bin,
        ...
        if len(cell_contour) > 0:
            ...
            if len(contour_list) >= 1:
                ...
                cv2.rectangle(img=cell_rescaled, 
                                pt1=(x_c, y_c), 
                                pt2=(x_c+w_c, y_c+h_c), 
                                color=(0,0,255))
